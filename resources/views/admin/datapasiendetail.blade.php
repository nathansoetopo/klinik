...
@section('content')
    ...
    <script>
        ...
        var pasienID = {!! json_encode($pasienID) !!}
        console.log(data)
        $(document).on('click', '#addForm', function() {
            ++i;
            var element = '<div class="container">' +
                '<input type="hidden" name="obat[' + i + '][pasien_id]" value="' + pasienID + '"' +
                '<label for="obat">Obat</label>' +
                '<div class="form-group">' +
                '<textarea class="form-control" id="obat" name="obat[' + i +
                '][desc]" placeholder="Masukkan Obat" style="height:100px;"></textarea>' +
                '</div>' +
                '<div class="form-row">' +
                '<div class="form-group col-md-7">' +
                '<label for="inputObat">Nama obat</label>' +
                '<select class="form-control" id="inputObat' + i + '" name="obat[' + i + '][medicine_id]">' +
                '</select>' +
                '</div>' +
                '<div class="form-group col-md-5">' +
                '<label for="inputState">Jumlah</label>' +
                '<input type="number"  class="form-control" min="1" name="obat[' + i + '][jml_obat]" required>' +
                '</div>' +
                '</div>' +
                '<div class="form-row">' +
                '<div class="form-group col-md-7">' +
                '<label for="aturan">Aturan pakai</label>' +
                '<select class="form-control" name="obat[' + i + '][aturan_pakai]" id="aturan">' +
                '<option>1</option>' +
                '<option>2</option>' +
                '<option>3</option>' +
                '<option>4</option>' +
